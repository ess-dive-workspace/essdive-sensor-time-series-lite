...
|additional guidance|For any files following the Sensor Time Series - Lite reporting format, including data, methods and attributes, and data dictionary, provide "ESS-DIVE Sensor Time Series - Lite Reporting Format v1" in the `standard` term.|
...
|format|free text|
|unit|N/A|
|definition|Column name from the data file. Provide entries for each column name from the data matrix in the data file.|
|example|temp_soil_2|
...
|:----------------------------------------------------|:----------------------------------------------------|
|requirement|required|
|format|[Controlled vocabulary](https://github.com/ess-dive-workspace/essdive-sensor-time-series-lite/blob/release-v1.0.0/controlled_vocabulary.md#material_measured)|
|unit|N/A|
...
|additional guidance|`latitude` and `longitude` is strongly recommended rather than using only the `location_description` term.|
...
|format|numeric|
|unit|meter|
|definition|Height / altitude (positive numbers) or depth (negative numbers) in reference to the ground surface, unless a different reference is specified in the `vertical_reference_position` term. One vertical position is allowed for each location.|
|example|-0.25|
|additional guidance|N/A|
...
|example|water_bed|
|additional guidance|If your `vertical_position_reference` is not in the controlled vocabulary, choose "other" and provide the vertical position reference in the notes term.|
